...
运行成功后会将简道云成员、部门信息写入到配置文件对应的表名中。程序会自动创建对应表，每次更新都是全量更新。
...
为了减少请求次数，会缓存表单结构数据。在字段名称、顺序发生变化时，程序会自动刷新结构数据缓存。

但如果删除表单字段后重新添加同名字段，对应简道云字段 `id` 发生变化，需要通过修改 `etag` 值，让程序更新表单组件缓存数据。
...
以实际例子说明，假设简道云表单设置字段有`单行文本`、`数字`、`唯一字段`三个字段，那么生成的队列消息内容如下：
...
程序根据 `query_fields` 中的字段，找到 `data` 中 `唯一字段` 的值，然后转换为简道云 API 需要的 `_widget_xxxxx` 字段对应 ID。
查询符合条件的表单。再根据 `op` 与 `safe_limie` 的配置通过 API 操作表单。

基于以上运行逻辑，所以不能支持表单中存在多个同名字段。
